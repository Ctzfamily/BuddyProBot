...
import buddy.modules.sql.global_bans_sql as sql
from buddy import dispatcher, OWNER_ID, DEV_USERS, SUDO_USERS, SUPPORT_USERS, WHITELIST_USERS, GBAN_LOGS, STRICT_GBAN, spam_watch
from buddy.modules.helper_funcs.chat_status import user_admin, is_user_admin
from buddy.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from buddy.modules.helper_funcs.filters import CustomFilters
from buddy.modules.helper_funcs.misc import send_to_list
from buddy.modules.sql.users_sql import get_all_chats
...
def gban(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text("என் தலைவரை என்னால் எப்படி BAN செய்ய முடியும்?😔")
        return

    if int(user_id) in SUDO_USERS:
        message.reply_text("இவர் என் SUDO USER போட வெண்ண🤬😏")
        return

    if int(user_id) in SUPPORT_USERS:
        message.reply_text("இவர் என் SUPPORTER போட வெண்ண🤬😏")
        return
    
    if int(user_id) in WHITELIST_USERS:
        message.reply_text("I can't ban my owner's close frd.")
        return
    ...
    message.reply_text("⚡️ *அப்பாலே போ சாத்தானே* ⚡️")
