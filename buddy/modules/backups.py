...
import buddy.modules.sql.notes_sql as sql
from buddy import dispatcher, LOGGER, OWNER_ID, SUDO_USERS, MESSAGE_DUMP
from buddy.__main__ import DATA_IMPORT
from buddy.modules.helper_funcs.chat_status import user_admin
from buddy.modules.helper_funcs.misc import build_keyboard, revert_buttons
from buddy.modules.helper_funcs.msg_types import get_note_type
from buddy.modules.rules import get_rules
import buddy.modules.sql.rules_sql as rulessql
from buddy.modules.sql import warns_sql as warnssql
import buddy.modules.sql.blacklist_sql as blacklistsql
from buddy.modules.sql import disable_sql as disabledsql
from buddy.modules.sql import cust_filters_sql as filtersql
import buddy.modules.sql.welcome_sql as welcsql
import buddy.modules.sql.locks_sql as locksql
...
def export_data(bot: Bot, update: Update, chat_data):
    ...
    f=open("buddy{}.backup".format(chat_id), "w")
    ...
    try:
        ...
        pass
    bot.sendDocument(current_chat_id, document=open('buddy{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This  is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
    os.remove("buddy{}.backup".format(chat_id)) # Cleaning file
